...
static void char_constant(struct token *t, node_t * sym)
{
    ...
    if (!char_rec && !len)
        cc_error("incomplete character constant: %s", t->lexeme);
    else if (overflow)
        ...
        cc_error("character constant overflow: %s", t->lexeme);
...
static void integer_constant(struct token *t, node_t * sym)
{
    ...
    if (s[0] == '0' && (s[1] == 'x' || s[1] == 'X')) {
        ...
        if (err)
            cc_error("invalid octal constant %s", t->lexeme);
    } else {
        base = 10;
        for (; isdigit(*s);) {
            int d = *s - '0';
    ...
    case INT:
        if (overflow || n > INTEGER_MAX(longlongtype))
            cc_error("integer constant overflow: %s", t->lexeme);
        SYM_VALUE_I(sym) = n;
        break;
    case UNSIGNED:
        if (overflow)
...
static void number_constant(struct token *t, node_t * sym)
{
    ...
    if (pc[0] == '\'' || pc[0] == 'L') {
        ...
        pc += 2;
        if (!isxdigit(*pc) && pc[0] != '.') {
            cc_error("incomplete hex constant: %s", t->lexeme);
...
static node_t *primary_expr(void)
{
    ...
    case ID:
        ...
        if (sym) {
            ...
            cc_error("use of undeclared identifier '%s'", token->lexeme);
        }
        expect(t);
        break;
    case NCONSTANT:
    ...
    case '(':
        ...
        break;
    default:
        cc_error("invalid postfix expression at '%s'", token->lexeme);
        break;
    }
